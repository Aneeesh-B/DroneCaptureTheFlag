...
def run_scenario(scenario, result_queue, alpha1=10, alpha2=0.1, vel_weight=0.5, avoidance=4.0):
...
def monte_carlo(num_iterations=50):
    ...
    for i in range(0, num_iterations, max_parallel_processes):
        ...
        for _ in range(batch_size):
            ...
            p = mp.Process(target=worker, args=(alpha1, alpha2, vel_weight, avoidance, i, result_queue))
            p.start()
            processes.append(p)
...
if __name__ == "__main__":
    ...
    # if os.path.exists(csv_filename):
    #     os.remove(csv_filename)  # Clear previous data if file exists
